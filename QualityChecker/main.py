...
checks = [1, 2, 3, 4, 5, 6, 7, 8, 10, 11, 9]
# 1. Дубли по ключам
...
df_list1 = pandas.DataFrame(
    {'schema': schema_df, 'table': table_df, 'pk_doubles': check_pk_doubles_df,
     'max_ts_ods': check_max_tech_load_ts_df, 'null_fields': count_null_cols_df,
     'max_length': count_max_length_df,
     'not_utf8': count_not_utf8_cols_df, 'stg_row_count': check_row_count_stg_df,
     'ods_row_count': check_row_count_ods_df,
     'segmentation': check_segmentation_df})
df_list2 = pandas.DataFrame({'schema': col_schema_df, 'table': col_table_df, 'column': col_df, 'null_cols': not_null_df,
                             'not_utf8': not_utf8_df, 'max_length': max_length_df, 'Consist': stat_most_cons_val_df,
